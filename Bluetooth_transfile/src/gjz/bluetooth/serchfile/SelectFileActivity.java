...
import gjz.bluetooth.BluetoothApplication;
...
public class SelectFileActivity extends Activity {
	...
	private File file;   //当前操作文件 �?文件�?
	
	private String sdcardPath;  //sd卡路�?
	private String path;    //当前文件父目�?
	
	Button mBackBtn;  //返回按钮
	Button mEnsureBtn;   //确定按钮
	Button mCancelBtn;   //取消按钮

	TextView mLastClickView;   //�?���?��点击的文�?--文件�?
	TextView mNowClickView;   //现在点击的文�?-- 文件�?
	private boolean isSelected = false;   //是否选择了文�?  (非文件夹)
	...
	private OnClickListener mEnsureBtnClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			if(!isSelected){
				//没有选择文件
				Toast.makeText(SelectFileActivity.this, "请�?择文�?", Toast.LENGTH_LONG).show();
				return ;
			}
			//将�?择的文件的全路径 返回
			Intent intent = new Intent();
			intent.putExtra("filepath", file.getAbsolutePath());
			SelectFileActivity.this.setResult(11, intent);
			SelectFileActivity.this.finish();
		}
	};
	
	/**
	 * 根据父目录path显示path下所有文件及文件�?
	 */
